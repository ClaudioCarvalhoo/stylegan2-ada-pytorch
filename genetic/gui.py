...
class LatentHiker(tk.Frame):
    def __init__(self, parent, population_size, G, *args, **kwargs):
        ...
        photo1 = ImageTk.PhotoImage(
            Image.open("./genetic/gato_pizza.jpg").resize((256, 256))
        )
        ...
        for i, individual in enumerate(self.population.individuals):
            ...
            generated_image = ImageTk.PhotoImage(
                generate_image(G, individual.genes).resize((256, 256))
            )
            ...
            slider = tk.Scale(
                self, from_=0, to=10, orient=tk.HORIZONTAL, bg="black", fg="white"
            )
            slider.grid(row=row + 1, column=col, pady=(5, 25))
            self.sliders.append(slider)
        row += 2

        tk.Label(self, text="Mutation Rate:", bg="black", fg="white").grid(
            row=row, column=0
        )
        row += 1
        ...
        tk.Button(
            self, text="UPDATE", width=16, bg="white", fg="black", command=self._update
        ).grid(row=row, column=cells_per_row)
        row += 1
    ...
    def _update(self):
        ...
        for i in range(len(self.population.individuals)):
            generated_image = ImageTk.PhotoImage(
                generate_image(self.G, self.population.individuals[i].genes).resize(
                    (256, 256)
                )
            )
...
def start_gui(population_size, G):
    ...
    canvas.bind_all(
        "<MouseWheel>", lambda e: canvas.yview_scroll(-1 * int((e.delta / 60)), "units")
    )
